...
$user = requireLogin();
$orgId = getAuthOrgId();
$fromTopic = trim((string)($_GET['from_topic'] ?? ''));

$fromTopicQuestions = [];
if (!empty($fromTopic)) {
    $db = Database::getConnection();
    $stmtTQ = $db->prepare("
        SELECT q.id, q.question_text, q.timer_seconds, q.max_points, q.difficulty, q.topic
        FROM questions q
        ...
        ORDER BY q.id ASC
    ");
    $stmtTQ->execute(['org_id' => $orgId, 'topic' => $fromTopic]);
    $fromTopicQuestions = $stmtTQ->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($fromTopicQuestions as &$ftq) {
        $stmtOpts->execute(['qid' => $ftq['id']]);
        $opts = $stmtOpts->fetchAll(PDO::FETCH_ASSOC);
        $ftq['options'] = [];
        $ftq['correct_option'] = 'A';
        foreach ($opts as $opt) {
            $ftq['options'][$opt['option_key']] = $opt['option_text'];
            if (!empty($opt['is_correct'])) {
                $ftq['correct_option'] = $opt['option_key'];
            }
        }
    }
    unset($ftq);
...
<head>
    ...
    <title>Quiz Builder — <?= APP_NAME ?></title>
...
<body>
    ...
    <div class="container" style="max-width: 920px;">
        <div style="margin-bottom: 2rem;">
            ...
            <h1 style="font-size: 2.2rem; font-weight: 800; color:var(--text-primary); margin-top:0.25rem;">Interactive Quiz Builder</h1>
        </div>
        ...
        <form id="quizBuilderForm">
            <div class="card" style="margin-bottom: 2rem;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="quiz_title" class="form-input" placeholder="e.g. Data Structures & Algorithms Quiz" value="<?= htmlspecialchars($fromTopic ? $fromTopic . ' Quiz' : '', ENT_QUOTES) ?>" required style="height: 48px;">
                </div>

                <div class="form-group">
                    <label class="form-label">Description</label>
                    <textarea id="quiz_description" class="form-textarea" rows="3" placeholder="Brief explanation of quiz topic"><?= htmlspecialchars($fromTopic ? "Live assessment on {$fromTopic}" : '') ?></textarea>
                </div>

                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                    <div class="form-group">
                        <label class="form-label">Category</label>
                        <input type="text" id="quiz_category" class="form-input" value="<?= htmlspecialchars($fromTopic ?: 'General', ENT_QUOTES) ?>">
                    </div>
                    <div class="form-group">
            ...
            <div class="card">
                ...
                <div style="margin-top: 2rem;">
                    <button type="submit" class="btn btn-primary btn-lg btn-full" style="border-radius:14px;">Save & Publish Quiz 🚀</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        // Preload questions if from_topic was provided, otherwise add 1 empty card
        const preloadedTopicQuestions = <?= json_encode($fromTopicQuestions) ?>;
        if (Array.isArray(preloadedTopicQuestions) && preloadedTopicQuestions.length > 0) {
            preloadedTopicQuestions.forEach(q => {
                addQuestionCard(q);
            });
            QuizlyApp.showAlert('quizAlert', `Pre-loaded ${preloadedTopicQuestions.length} questions from topic "${QuizlyApp.escapeHtml('<?= addslashes($fromTopic) ?>')}".`, 'success');
        } else {
            addQuestionCard();
        }
        ...
        document.getElementById('quizBuilderForm').addEventListener('submit', async (e) => {
            ...
            if (qCards.length === 0) {
                QuizlyApp.showAlert('quizAlert', 'Please add at least one question to the quiz.', 'danger');
                return;
            }
            ...
            try {
                ...
                const res = await QuizlyApp.fetchJson('<?= BASE_URL ?>/api/quizzes.php?action=save', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });

                QuizlyApp.showAlert('quizAlert', 'Quiz created successfully! Redirecting to dashboard...', 'success');
                setTimeout(() => {
        ...
        function applyImportedQuestions() {
            ...
            // If only 1 question was blank, replace it
            const firstPrompt = document.querySelector('.q-text');
            if (questionCount === 1 && firstPrompt && !firstPrompt.value.trim()) {
                const firstCard = document.getElementById('qCard_1');
